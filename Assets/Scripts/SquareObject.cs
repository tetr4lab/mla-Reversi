	...
	public class SquareObject : MonoBehaviour, IPointerClickHandler {
		...
		public void RequestUpdate () {
			if (animator) {
				...
				stepText.text = (!(BoardObject.AllowDisplayHint || square.Step == game.Step) || square.Step < 0) ? "" : $"<color={((square.Step == game.Step) ? "red" : square.IsBlack ? "white" : "black")}>{square.Step}</color>";
			}
		}
